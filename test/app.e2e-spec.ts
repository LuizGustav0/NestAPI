...
describe('AppController (e2e)', () => {
  ...
  it('Obter dados do usuário logado', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
      .set('Authorization', `beareare ${accessToken}`)
      .send();
  ...
  it('Registrar um novo usuário como admin', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/register')
      .send({...authRegisterDTO, role: Role.Admin, email: 'useradmin@hotmail.com'});
  ...
  it('Validar a função do novo usuário ainda é User', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
      .set('Authorization', `beareare ${accessToken}`)
      .send();
  ...
  it('Tentar ver a lista de todos os usuários com acesso administrador', async () => {
    const response = await request(app.getHttpServer())
      .get('/users')
      .set('Authorization', `beareare ${accessToken}`)
      .send();
